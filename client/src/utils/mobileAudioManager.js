...
class MobileAudioManager {
  ...
  loadDynamicSettings() {
    ...
    console.log('📱 Mobile audio settings updated:', {
  ...
  // Enable/disable with persistence - now syncs with settings service
  setEnabled(enabled) {
    ...
    if (enabled) {
      console.log('🔊 Mobile audio alerts ENABLED at current volume');
    } else {
      console.log('🔇 Mobile audio alerts DISABLED');
    }
  }

  // Force maximum volume - now syncs with settings service
  setMaxVolume() {
    this.volume = 1.0;
    
    // Update the settings service to 100%
    settingsService.updateSetting('notify.sound_volume', 100);
