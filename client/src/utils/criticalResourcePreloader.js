...
class CriticalResourcePreloader {
  constructor(tableCount = 25) {
    this.tableCount = tableCount;
    this.preloadedResources = new Set();
    this.criticalImages = [
      '/images/Food Zone Restaurant Logo.jpg'
    ];
    // Fonts are preloaded via <link> tags in public/index.html — no JS preload here
    // to avoid duplicate-preload / crossorigin-mismatch warnings.
    this.criticalFonts = [];
  }
  ...
  async preloadCriticalResources() {
    ...
    try {
      ...
      if (isDev) console.log('✅ Critical resources preloaded for instant table loading');
    } catch (error) {
      if (isDev) console.log('⚠️ Some critical resources failed to preload:', error);
    }
  }
  ...
  // Preload critical API endpoints - disabled to prevent rate limiting
  async preloadCriticalAPIs() {
    if (isDev) console.log('🚫 API preloading disabled to prevent rate limiting');
  ...
  getNearbyTables(tableNumber) {
    ...
    for (let i = Math.max(1, num - 2); i <= Math.min(this.tableCount, num + 2); i++) {
  ...
  async preloadMenuPageResources() {
    // No hardcoded menu images — actual menu item images come from the API.
    return Promise.resolve();
  }
  ...
  getPreloadedAPIData(endpoint) {
    ...
    if (cached) {
      try {
        ...
        if (isDev) console.log('Failed to parse preloaded API data:', error);
      }
    }
