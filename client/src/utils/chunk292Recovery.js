...
class Chunk292Recovery {
      ...
      for (const cacheName of cacheNames) {
        ...
        for (const request of requests) {
          if (request.url.includes('292') || request.url.includes('.chunk.js')) {
            ...
            if (isDev) console.log(`🧹 Deleted ${request.url} from cache ${cacheName}`);
          }
        }
      }
    }
    ...
    if (window.__webpack_require__ && window.__webpack_require__.cache) {
      Object.keys(window.__webpack_require__.cache).forEach(key => {
        if (key.includes('292')) {
          ...
          if (isDev) console.log(`🧹 Cleared webpack cache for ${key}`);
        }
      });
    }
  }
    ...
    for (const url of chunkUrls) {
      try {
        ...
        if (response.ok) {
          ...
          if (isDev) console.log('✅ Successfully loaded chunk 292 via alternative method');
          return true;
        }
      } catch (error) {
  ...
  async preloadFallbackData() {
    // Preload basic menu data as fallback - disabled to prevent rate limiting
    if (isDev) console.log('📦 Fallback menu data preloading disabled to prevent rate limiting');
  }
  ...
  triggerRecovery() {
    if (isDev) console.log('🔧 Manual chunk 292 recovery triggered');
  ...
  reset() {
    this.chunk292Failures = 0;
    this.recoveryActive = false;
    if (isDev) console.log('🔄 Chunk 292 recovery state reset');
  }
}
