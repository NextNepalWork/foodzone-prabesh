...
const Admin = () => {
  ...
  const [isAuthenticated, setIsAuthenticated] = useState(() => {
    ...
    // Security: Only allow admin users, not staff or reception
  ...
  useEffect(() => {
    ...
    newSocket.on('newOrder', (order) => {
      ...
      // Auto-refresh to ensure consistency
      console.log('🔄 Auto-refreshing orders after new order...');
      fetchOrders();
    ...
    newSocket.on('orderStatusUpdated', ({ orderId, status }) => {
      ...
      // Auto-refresh to ensure consistency
      console.log('🔄 Auto-refreshing orders after status update...');
      fetchOrders();
    });
    ...
    const refreshInterval = setInterval(() => {
      if (isAuthenticated) {
        console.log('🔄 Periodic refresh of admin orders...');
        fetchOrders();
  ...
  const handleLogin = async (e) => {
    ...
    try {
      ...
      const response = await fetchApi.post('/api/admin/auth', { username, password });
  ...
  const confirmCompleteOrder = async () => {
    ...
    try {
      ...
      // Auto-refresh to ensure consistency
      console.log('🔄 Auto-refreshing orders after completion...');
      await fetchOrders();
  ...
  const handlePaymentClick = (order) => {
    console.log('💳 Admin Payment button clicked for order:', order.id, order.order_number);
    console.log('Order details:', order);
    setSelectedOrder(order);
    setShowPaymentModal(true);
    console.log('Admin Payment modal should now be visible');
  };
  ...
  const confirmDeleteOrder = async () => {
    // Use proper authentication instead of hardcoded password
    ...
    if (deleteDialog.password !== 'DELETE') {
      alert('❌ Please type DELETE to confirm');
      return;
    }

    try {
      const response = await fetch(`${getApiUrl()}/api/order/${deleteDialog.orderId}`, {
        method: 'DELETE',
        headers: {
      ...
      if (result.success) {
        ...
        setOrderHistory(prevHistory => prevHistory.filter(order => order.id !== deleteDialog.orderId));
      ...
      <div className="min-h-screen bg-gradient-to-br from-gray-900 via-blue-900 to-purple-900 flex items-center justify-center px-4">
        <div className="bg-white rounded-2xl shadow-2xl p-8 w-full max-w-md">
          <div className="text-center mb-8">
            ...
            <p className="text-gray-600">Enter password to access {restaurantInfo.name} Admin Panel</p>
          </div>
          ...
          <div className="mt-6 text-center">
            <p className="text-xs text-gray-500">
              🍽️ {restaurantInfo.name} Restaurant Management System
            </p>
          </div>
        </div>
      </div>
          ...
          <div className="bg-white rounded-lg p-6 max-w-md mx-4">
            ...
            <div className="flex gap-3 justify-end">
              ...
              <button
                ...
                disabled={deleteDialog.password !== 'DELETE'}
                className={`px-4 py-2 rounded transition-colors ${
                  deleteDialog.password === 'DELETE'
      ...
      <div className="container mx-auto px-4 py-8">
        <div className="bg-white rounded-lg shadow-md p-6">
          <div className="flex justify-between items-center mb-6">
            <h1 className="text-3xl font-bold text-gray-800">🍽️ {restaurantInfo.name} Admin Panel</h1>
                ...
                <div key={order.id} className="bg-white rounded-lg shadow-md p-6 border-l-4 border-primary">
                  <div className="flex justify-between items-start mb-4">
                    <div>
                      ...
                      <p className="text-gray-600">👤 {order.customer_name || 'Guest'} • 📞 {order.customer_phone || 'N/A'}</p>
                      ...
                      <div className="mt-2 space-y-2">
                        ...
                        {order.status === 'completed' && (
                          <div className="space-y-2">
                            ...
                            {order.payment_status !== 'paid' && (
                              <button
                                onClick={(e) => {
                                  e.preventDefault();
                                  e.stopPropagation();
                                  console.log('Admin Button clicked - event:', e);
                                  handlePaymentClick(order);
                                }}
                                className="w-full bg-purple-600 hover:bg-purple-700 text-white px-4 py-2 rounded-lg text-sm font-medium transition-colors duration-200 shadow-sm"
                            ...
                            {order.payment_status === 'paid' && (
                              <div className="w-full bg-blue-100 text-blue-800 px-4 py-2 rounded-lg text-sm font-medium text-center border border-blue-200">
                                💰 Paid
                              </div>
                            )}
                          </div>
      ...
      {activeTab === 'database' && (
          <div className="bg-white rounded-lg shadow-sm border border-gray-200">
            <div className="p-6 border-b border-gray-200">
              ...
              <p className="text-gray-600">Manage and clear test data from the {restaurantInfo.name} database</p>
            </div>
                  ...
                  {customers.map((customer) => (
                    <tr key={customer.id} className="hover:bg-gray-50">
                      <td className="px-6 py-4 whitespace-nowrap">
                        <div className="text-sm font-medium text-gray-900">{customer?.name || 'Guest'}</div>
                        {customer?.email && (
                      ...
                      <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                        {customer?.actual_order_count || customer?.total_orders || 0}
                      </td>
                      <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                        NPR {parseFloat(customer?.total_spent || 0).toFixed(2)}/-
                      </td>
