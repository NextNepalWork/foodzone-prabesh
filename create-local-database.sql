...
DROP DATABASE IF EXISTS foodzone_local;

-- Create the database
CREATE DATABASE foodzone_local;

-- Connect to the new database
\c foodzone_local;

-- Food Zone Database Schema (Modified for local setup without PostGIS)
...
CREATE TABLE orders (
    ...
    -- Table info (for dine-in orders) - Modified to support VARCHAR
    ...
    -- Order status - Updated with new statuses
...
-- 8. Payment Daybook tables
CREATE TABLE payment_daybook (
    id SERIAL PRIMARY KEY,
    date DATE NOT NULL DEFAULT CURRENT_DATE,
    opening_balance DECIMAL(10,2) DEFAULT 0,
    total_cash_payments DECIMAL(10,2) DEFAULT 0,
    total_online_payments DECIMAL(10,2) DEFAULT 0,
    total_expenses DECIMAL(10,2) DEFAULT 0,
    cash_handovers DECIMAL(10,2) DEFAULT 0,
...
CREATE TABLE payment_transactions (
    id SERIAL PRIMARY KEY,
    daybook_id INTEGER REFERENCES payment_daybook(id) ON DELETE CASCADE,
    order_id INTEGER REFERENCES orders(id),
    transaction_type VARCHAR(20) NOT NULL CHECK (transaction_type IN ('cash_payment', 'online_payment', 'expense', 'handover')),
    amount DECIMAL(10,2) NOT NULL,
...
('restaurant_name', 'Food Zone Local', 'Restaurant name'),
('restaurant_phone', '9851234567', 'Primary contact number'),
('restaurant_address', 'Local Development Environment', 'Restaurant address'),
...
('Local Area', 'Local development area', 30, 15, 1.0, 150),
('Extended Area', 'Extended local area', 50, 25, 3.0, 200),
('City Center', 'City center area', 80, 35, 5.0, 300);
...
CREATE TRIGGER update_payment_daybook_updated_at BEFORE UPDATE ON payment_daybook
    FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();
...
-- Success message
SELECT 'Local Food Zone database created successfully!' as message;
